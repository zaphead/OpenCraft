...
pub fn grow(chunk: &mut Chunk, seed: i64) {
    ...
    for lz in 0..16 {
        for lx in 0..16 {
            let (x, z) = (ox + lx, oz + lz);
            if biome_at(seed, x, z) != Biome::Meadow { continue; }
            let y = ground(seed, x, z);
            if get(chunk, x, y - 1, z) != blocks::GRASS { continue; }
            let h = hash(seed, x, z);
            if h % 5 == 0 { put(chunk, x, y, z, blocks::FLOWER); }
            else if h % 5 == 1 { put(chunk, x, y, z, blocks::SUNPETAL); }
            else if h % 7 == 0 { put(chunk, x, y, z, blocks::TUFT); }
            if h % 11 == 0 { put(chunk, x, y, z, blocks::FLOWER); }
        }
    }
}
